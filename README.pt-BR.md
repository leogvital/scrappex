...
- **Rate limit**: `/api/auth/app-login` permite `_LOGIN_RATE_LIMIT` (5) tentativas erradas por IP dentro de `_LOGIN_RATE_WINDOW` (300s), depois retorna `429` — inclusive pra senha *certa*, até a janela liberar — com uma mensagem "aguarde Xs" que o frontend já mostra através do estado `err` existente (nenhuma mudança no frontend foi necessária; `fetch` não levanta exceção num 429, então `d.error` do corpo JSON renderiza exatamente como uma mensagem de senha errada). Rastreado só em memória (`_LOGIN_ATTEMPTS`, um dict simples — sem Redis), já que `--workers 1` significa que não há estado entre processos pra sincronizar de qualquer jeito, e perder o contador num restart é inofensivo aqui, não um buraco de segurança. Um login certo limpa o histórico de falhas daquele IP na hora. `_login_rate_limited()` remove tentativas mais antigas que a janela como efeito colateral de cada checagem, então tanto a lista por IP quanto o dict em si (uma vez que a lista de uma chave esvazia) ficam naturalmente limitados — sem job de limpeza separado. Validado com 7 casos: a 5ª tentativa ainda passa e a 6ª é bloqueada (o off-by-one é proposital — a janela enche nas tentativas 1-5, então a checagem na tentativa 6 é a primeira a ver `len >= 5`), um IP bloqueado não consegue passar nem com a senha certa, um IP diferente não é afetado, um login certo limpa o histórico, uma nova sequência de 5 falhas é exigida pra disparar de novo depois de um reset, e tentativas fora da janela de tempo são removidas e param de contar.
...
Um 6º botão no seletor de plataforma, "🌐 Tudo", entra no `searchAllMode`: uma única caixa de busca pesquisa X, xHamster, XVideos, xFree e Pornhub com `searchAllPlatforms()`. Cada chamada `/api/search` de cada plataforma roda **sequencialmente dentro de um `for`, não com `Promise.all`** — isso é proposital, não um esquecimento. Diferente do X e do xFree (cada um com seu próprio dicionário de sessão dedicado, `_SS`/`_XF_SS`), xHamster, XVideos e Pornhub são scrapers HTTP puros que compartilham a mesma infraestrutura `_SITE_SS`; disparar duas dessas três plataformas ao mesmo tempo faria a atualização de sessão da segunda resposta atropelar o estado de paginação em andamento da primeira no meio da escrita. Rodar tudo sequencialmente evita essa corrida inteira em vez de decidir caso a caso quais pares de plataforma seriam seguros de paralelizar.

Os resultados vão aparecendo conforme cada plataforma termina (`setAllResults(prev => [...prev, ...tagged])` depois de cada plataforma, não tudo de uma vez no final) em vez de esperar as 5 antes de mostrar qualquer coisa. Cada resultado é marcado com `_platform` na entrada; o `VideoCard` renderiza uma badge pequena a partir de `PLATFORM_LABEL` quando esse campo está presente, e fora isso não muda em nada — o card, o `FormatModal` e todo o fluxo de download sempre dependeram só de `video.url`, nunca do estado `platform` externo, então misturar resultados de cinco plataformas diferentes numa mesma grade não precisou de mudança nenhuma ali.

**"Carregar mais" por plataforma**: `allSearchIds`/`allHasMore`/`allLoadingMore` rastreiam o `search_id` e o estado de paginação de cada plataforma de forma independente, e `loadMoreAllPlatform(platform)` chama `/api/search/more` com o `search_id` específico daquela plataforma. Isso só ficou confiável depois que o `_SITE_SS` no backend deixou de ser um dict único e virou um dict indexado por site (`_SITE_SS["xhamster"]`, `_SITE_SS["xvideos"]`, `_SITE_SS["pornhub"]`), cada um localizado pelo `search_id` correspondente em vez de assumido como "o site que buscou mais recentemente". Com o slot único antigo, duas das três plataformas de site *sempre* teriam uma sessão já substituída no momento em que a busca agregada terminasse (qualquer uma que rodasse antes da última no loop sequencial), deixando o "carregar mais" delas permanentemente quebrado em vez de só ocasionalmente instável — isso não era visível antes do "buscar em tudo" existir, já que só uma plataforma tinha resultados na tela por vez.

**Seleção em lote no modo agregado**: um Set `checkedAll` separado do `checked` de plataforma única, indexado por `` `${platform}-${id}` `` em vez do id puro do vídeo — ids não têm garantia de serem únicos entre cinco sites não relacionados, então uma chave composta evita que a seleção de uma plataforma acidentalmente marque/desmarque um item de id igual em outra. O `VideoCard` ganhou uma prop opcional `checkKey` (cai pro `video.id` se não informada) pra que o mesmo componente atenda os dois modos sem duplicação. `startBulkDownloadAll()` reaproveita exatamente o mesmo pipeline `bulkTasks`/`BulkProgressModal` do download em lote de plataforma única — ele só precisa de `url`/`title`/`uploader`/`fmt` por item, que os resultados agregados já carregam independente da plataforma de origem.
...
Validado estendendo o mesmo teste de render headless: trocar pro modo agregado, submeter uma busca, confirmar que exatamente um POST `/api/search` dispara por plataforma (cinco no total, um pra cada x/xhamster/xvideos/xfree/pornhub), confirmar que os resultados das cinco plataformas renderizam juntos com a contagem certa no resumo, confirmar que a execução vira uma única entrada no histórico, confirmar que um botão "carregar mais" específico de uma plataforma busca e adiciona mais resultados e some quando aquela plataforma se esgota, e confirmar que selecionar todos os resultados agregados e baixar em lote dispara um `/api/download/start` por item.
...
O yt-dlp não tem "pausar" nativo. O truque: `FileDownloader._hook_progress()` do yt-dlp chama todo `progress_hook` registrado **sem nenhum `try/except` ao redor** — qualquer exceção que um hook levante propaga direto pra fora do `extract_info()`, sem ser capturada. O hook do `download_task()` checa um `threading.Event` por task (`_download_control[task_id]["stop"]`) em toda chamada; quando setado, ele levanta uma exceção, e os próprios blocos `except` do `download_task` distinguem *o motivo* via `_download_control[task_id]["action"]` (`"pause"` vs `"cancel"`, setado por `/api/download/pause/<tid>` / `/api/download/cancel/<tid>` logo antes de setar o evento):

- **Pausar** deixa o arquivo `.part` intocado no disco. **Retomar** não é nada além de chamar `/api/download/start` de novo com o mesmo `url`/`format_id` — como o `outtmpl` (`%(title)s_%(id)s.%(ext)s`) é determinístico, o yt-dlp cai exatamente no mesmo caminho de destino, encontra o `.part` existente, e continua via requisições HTTP Range usando o próprio comportamento padrão de resume (`continuedl=True`) — nenhum código de resume customizado foi necessário.
- **Cancelar** faz o mesmo sinal, mas o handler `except` remove o `.part` (e o `.ytdl`, no caso de downloads fragmentados) via `_remove_partial_files()`, usando o nome de arquivo exato que o hook reportou por último (`d["filename"]`) — não um glob no diretório inteiro, então o arquivo parcial de um download concorrente e não-relacionado nunca corre risco.
- Cancelar uma task **já pausada** (sem thread pra sinalizar — o `download_task` já retornou, `_download_control[tid]` foi removido no `finally`) é tratado como um branch separado em `/api/download/cancel/<tid>`: apaga o arquivo registrado em `download_progress[tid]["filename"]` direto, sem envolver sinalização nenhuma.

Controles de pausar/cancelar/retomar aparecem em todo lugar que mostra progresso de download — `FormatModal` (download único), `BulkProgressModal` (em lote) e `BgTray` (background) — cada um gerenciando seu próprio polling de forma independente, mas batendo nos mesmos três endpoints. `bgTasks`/`localStorage` (`scrapperx_bg_tasks`) agora também persistem `url`/`format_id`/`prog` por task (antes só `taskId`/`title`), que é o que torna um download pausado retomável mesmo depois de reabrir o navegador.

Validado com um teste em nível Python que simula o `yt_dlp.YoutubeDL` pra acionar o hook diretamente (sem rede real): pausar no meio do download preserva o `.part` e registra `url`/`format_id` pra retomar; cancelar no meio do download remove ele; cancelar uma task já pausada remove pelo caminho sem thread; e reinvocar `download_task` com o `url`/`format_id` de uma task pausada completa normalmente. Também validado de ponta a ponta pela UI real com o mesmo harness de render headless usado pro histórico de busca: iniciar um download, pausar (a barra de progresso e os botões mudam pro estado "Pausado"), retomar (dispara um `/api/download/start` novo e volta pra "Baixando"), e cancelar (mostra "Cancelado" na hora).

### Histórico de downloads duplicados (sobrevive a apagar o arquivo)

`download_history.db` (SQLite, ao lado do `app.py` — propositalmente *não* embaixo de `tempfile.gettempdir()`, já que o objetivo inteiro é lembrar que um vídeo já foi baixado muito depois do arquivo em si ter sido apagado, o que descarta qualquer lugar que se apague num reboot ou numa limpeza de `/tmp` do sistema operacional; veja a correção do vazamento de disco no `ROADMAP.md` pra entender por que isso não é hipotético aqui) registra toda download concluído por URL, junto com título, nome do arquivo e tamanho. É um histórico *append-only* (não uma linha por URL), então baixar a mesma URL de novo depois — inclusive uma segunda vez depois de apagar a primeira cópia — continua totalmente registrado em vez de sobrescrever silenciosamente a entrada anterior.

`/api/download/start` checa esse histórico por URL antes de fazer qualquer coisa; se tiver uma correspondência, devolve `{"already_downloaded": true, "info": {...}}` em vez de começar um download novo, a menos que a requisição inclua explicitamente `"force": true`. Nada fica bloqueado pra sempre automaticamente — o `FormatModal` mostra o título/tamanho/data registrados e um botão "Baixar mesmo assim" que repete a mesma chamada com `force:true`; o `BulkProgressModal` faz o mesmo por item (identificado pelo índice no array em vez do `taskId`, já que um item "já baixado" que nunca chegou a começar não tem task nenhuma ainda — vários poderiam colidir numa chave `null` do contrário). Resultados de busca também são marcados: `_mark_downloaded(results)` roda uma única query em lote `WHERE url IN (...)` (não uma consulta por resultado) em todo ponto em que `/api/search`/`/api/search/more` devolve resultados, adicionando `already_downloaded: bool` a cada item — o `VideoCard` mostra uma badge pequena "✓ Já baixado" quando setado. Como `/api/search/start_all` reaproveita a mesma view `search()` por dentro (veja [Busca em Background](#busca-em-background)), os resultados agregados ganham a mesma marcação automaticamente, sem código extra.

O `download_task()` registra o histórico na conclusão — depois de confirmar que o arquivo existe no disco, lê o tamanho real com `os.path.getsize()` e insere `(url, título, nome_do_arquivo, tamanho_em_bytes, baixado_em)`. Uma conexão `sqlite3.connect()` nova a cada chamada (não uma conexão compartilhada) é o padrão seguro aqui, já que o gunicorn roda 4 threads de requisição (`--worker-class gthread`) e conexões sqlite3 não são seguras pra compartilhar entre threads por padrão.

Validado com um teste Python cobrindo: uma download registrado é encontrado por busca de URL e persiste independente do arquivo ainda existir no disco; uma URL sem histórico devolve `None`/inicia normalmente; a busca em lote usada pra marcar resultados de busca devolve exatamente as URLs correspondentes; `/api/download/start` bloqueia uma duplicata conhecida por padrão e prossegue com `force:true`; e o caminho real de conclusão do `download_task` (com um `yt_dlp.YoutubeDL` simulado, sem rede real) registra o título e tamanho reais do arquivo. Também validado pela UI: um resultado de busca mostra a badge "já baixado", abrir o modal de download e iniciar um download mostra o aviso com o tamanho registrado em vez de baixar, e "Baixar mesmo assim" repete com `force:true` e prossegue normalmente.

### Preview na biblioteca

Cada card em "Meus Vídeos" antes mostrava só um ícone de play estático num fundo vazio — nenhuma indicação de qual vídeo era sem abrir ele. O `LibraryCard` agora renderiza um `<video preload="metadata">` apontando direto pro `/api/library/video/<name>` (o mesmo endpoint de streaming com suporte a range requests que o `PlayerModal` em tela cheia já usava) — só o `preload="metadata"` já faz o navegador exibir o primeiro frame como thumbnail estática sem baixar o arquivo inteiro, mesmo antes de qualquer interação, e passar o mouse por cima toca o vídeo (mudo, em loop) pra um preview de verdade. Diferente do fluxo de preview remoto do `VideoCard`, que precisa de um round-trip `POST /api/preview` pra resolver uma URL reproduzível de um vídeo hospedado no site de outra pessoa, o arquivo de um item da biblioteca já *é* a URL reproduzível direta, então não tem requisição extra nenhuma envolvida.

Validado com um teste de render headless (o jsdom não implementa decodificação real de mídia, então `HTMLMediaElement.prototype.play`/`pause` são espionados em vez de exercitados de verdade): cada card da biblioteca renderiza um `<video>` apontando pra URL de streaming correta, passar o mouse por cima chama `.play()`, e tirar o mouse chama `.pause()`.
...
### Alerta quando um scraper para de retornar resultados silenciosamente

Uma exceção (erro de rede, erro de parse do HTML, crash do Selenium) já aparece na hora como mensagem de erro pro usuário — isso não é um buraco. O caso perigoso é o *silencioso*: a requisição "funciona" (200 OK, sem exceção) mas o parser não extrai nada, porque o site alvo mudou o formato do HTML/JSON por baixo de um seletor ou caminho de chave frágil. Uma única busca com zero resultados é normal (uma query rara, uma página momentaneamente vazia); várias **seguidas num endpoint que deveria confiavelmente ter conteúdo** é o sinal real de que algo quebrou.

`_record_scraper_outcome(platform, got_results, context)` rastreia uma sequência de resultados-zero consecutivos por plataforma em `_SCRAPER_HEALTH`, chamada em todo caminho de busca nova no `/api/search` (não no `/api/search/more` — paginação já tem sua própria semântica de "finished"/"has_more" que só adicionaria ruído aqui). Qualquer chamada com `got_results=True` zera a sequência daquela plataforma. Propositalmente restrito a **chamadas de home/categoria, não buscas por palavra-chave** — uma palavra-chave rara pode legitimamente dar zero com um parser 100% funcional, mas uma página inicial de categoria ou o próprio feed de uma conta ativa deveria sempre ter conteúdo:

- **xHamster / XVideos / Pornhub**: só é registrado quando `query` está vazia (navegação em home/categoria).
- **X**: só é registrado pra `type in ("home", "following")`, e exclui explicitamente o caso "X solicitou login" do `_NoResultsError` — isso é um problema de autenticação, não evidência de que a lógica de parsing de tweets quebrou.
- **xFree**: todo `_NoResultsError` conta — o xFree não tem tela de login, então uma página de categoria renderizando com zero elementos `.wall__item` não tem explicação legítima além de algo estar quebrado.

Quando a sequência de uma plataforma chega em `_SCRAPER_ZERO_ALERT_THRESHOLD` (5), loga uma linha `[PARSER-ALERT]` bem marcada (fácil de `grep` nos logs) — depois fica quieta até a sequência crescer mais `_SCRAPER_ZERO_ALERT_REPEAT` (10) falhas antes de alertar de novo, então um scraper que fica quebrado por horas não inunda o log a cada chamada, mas também não é esquecido. A mensagem propositalmente não assume um diagnóstico único ("parser quebrou" vs "site fora do ar") — os dois produzem o mesmo sintoma daqui, e o trabalho do alerta é só sinalizar "vai olhar", não adivinhar qual dos dois é.

Validado com 6 casos: nenhum alerta abaixo do limite, exatamente um alerta dispara no instante em que a sequência chega em 5, nenhum alerta repetido entre 5 e 15, um alerta repetido dispara exatamente em 15, um sucesso zera a sequência de modo que uma falha posterior precisa de 5 novas pra alertar de novo, e as plataformas são rastreadas de forma totalmente independente umas das outras.
