...
- **Preview nos cards da biblioteca** — antes cada card só mostrava um ícone de play num fundo vazio, sem indicação nenhuma de qual vídeo era. Agora usa `<video preload="metadata">` apontando pro streaming local (`/api/library/video/<name>`, já suporta range requests), então o navegador mostra o primeiro frame como thumbnail estática mesmo sem interação; passar o mouse toca o vídeo de verdade (mudo, em loop) pra um preview real. Diferente do preview de resultados de busca (`VideoCard`, que precisa de um POST em `/api/preview` pra resolver uma URL remota), um item da biblioteca já É o arquivo local, então não tem round-trip nenhum
- **Histórico de downloads persistente, sobrevive a apagar o arquivo** — `download_history.db` (SQLite, do lado do `app.py`, não em `/tmp` — o objetivo é lembrar mesmo muito depois, não só na sessão atual) registra toda busca+URL+título+tamanho num download concluído. `/api/download/start` verifica esse histórico por URL antes de começar; se já tiver um registro, devolve a informação (título, tamanho, data) em vez de baixar de novo, e só prossegue com um `force:true` explícito do usuário — clicando em "Baixar mesmo assim" depois de ver o aviso. Resultados de busca também ganham uma badge "✓ Já baixado" quando a URL já está no histórico. Como é um histórico (não uma linha por URL), baixar a mesma URL de novo depois de decidir "baixar mesmo assim" fica registrado também, sem perder o registro anterior
